...
@Component({
  selector: 'app-all-child-category-trash',
  templateUrl: './all-child-category-trash.component.html',
  styleUrl: './all-child-category-trash.component.scss',
  standalone: true,
  imports: [
    BreadcrumbComponent,
    FormsModule,
...
export class AllChildCategoryTrashComponent extends DataTableSelectionBase(Component) implements OnDestroy {
  ...
  private holdPrevUsers: ChildCategory[] = [];
  ...
  navArray: NavBreadcrumb[] = [
    ...
    {name: 'All ChildCategory', url: null},
  ];
  ...
  private getAllChildCategories() {
    ...
    this.subDataGetAll = this.childCategoryService.getAllChildCategories(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.childCategoryService.deleteMultipleChildCategoryById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleChildCategoryById(data: any) {
    this.subDataUpdateMulti = this.childCategoryService.updateMultipleChildCategoryById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
