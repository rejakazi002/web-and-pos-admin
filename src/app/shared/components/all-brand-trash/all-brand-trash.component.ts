...
@Component({
  selector: 'app-all-brand-trash',
  templateUrl: './all-brand-trash.component.html',
  styleUrl: './all-brand-trash.component.scss',
  standalone: true,
  imports: [
    BreadcrumbComponent,
    FormsModule,
...
export class AllBrandTrashComponent  extends DataTableSelectionBase(Component) implements  OnDestroy {

  // Decorator
  @ViewChild('searchForm') private searchForm: NgForm;

  // Store Data
  override allTableData: Brand[] = [];
  private holdPrevUsers: Brand[] = [];
  ...
  private getAllBrands() {
    ...
    this.subDataGetAll = this.brandService.getAllBrands(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.brandService.deleteMultipleBrandById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleBrandById(data: any) {
    this.subDataUpdateMulti = this.brandService.updateMultipleBrandById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
