...
@Component({
  selector: 'app-all-sub-category-trash',
  templateUrl: './all-sub-category-trash.component.html',
  styleUrl: './all-sub-category-trash.component.scss',
  standalone: true,
  imports: [
    BreadcrumbComponent,
    FormsModule,
...
export class AllSubCategoryTrashComponent extends DataTableSelectionBase(Component) implements  OnDestroy {
  ...
  private holdPrevUsers: SubCategory[] = [];
  ...
  private getAllSubCategories() {
    ...
    this.subDataGetAll = this.subCategoryService.getAllSubCategories(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.subCategoryService.deleteMultipleSubCategoryById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleSubCategoryById(data: any) {
    this.subDataUpdateMulti = this.subCategoryService.updateMultipleSubCategoryById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
