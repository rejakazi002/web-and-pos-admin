...
@Component({
  ...
  templateUrl: './all-order-trash.component.html',
  styleUrl: './all-order-trash.component.scss'
})
export class AllOrderTrashComponent extends DataTableSelectionBase(Component) implements OnInit, OnDestroy {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.orderService.deleteMultipleOrderById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleOrderById(data: any) {
    this.subDataUpdateMulti = this.orderService.updateMultipleOrderById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  openDetailsDialog(data:any): void {
    this.dialog.open(TableDetailsDialogComponent, {
      data: {data: data, nestedFieldName: 'name'},
      maxWidth: '800px',
      height: 'auto',
      maxHeight: '90vh'
    });
