...
@Component({
  selector: 'app-all-category-trash',
  standalone: true,
  imports: [
    BreadcrumbComponent,
    FormsModule,
  ...
  templateUrl: './all-category-trash.component.html',
  styleUrl: './all-category-trash.component.scss',

})
export class AllCategoryTrashComponent extends DataTableSelectionBase(Component) implements OnInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm') private searchForm: NgForm;

  // Store Data
  override allTableData: Category[] = [];
  private holdPrevUsers: Category[] = [];
  ...
  private getAllCategories() {
    ...
    this.subDataGetAll = this.categoryService.getAllCategories(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.categoryService.deleteMultipleCategoryById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCategoryById(data: any) {
    this.subDataUpdateMulti = this.categoryService.updateMultipleCategoryById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
