...
@Component({
  selector: 'app-all-pc-builder',
  templateUrl: './all-pc-builder.component.html',
  styleUrl: './all-pc-builder.component.scss'
})
export class AllPcBuilderComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: PcBuilder[] = [];
  ...
  private setPageData(): void {
    this.title.setTitle('PcBuilder');
    this.pageDataService.setPageData({
      title: 'PcBuilder',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'PcBuilder', url: null},
      ]
    })
  }
  ...
  private getAllPcBuilder() {
    ...
    const subscription = this.pcBuilderService.getAllPcBuilder(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultiplePcBuilderById() {
    const subscription = this.pcBuilderService.deleteMultiplePcBuilderById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultiplePcBuilderById(data: any) {
    const subscription = this.pcBuilderService.updateMultiplePcBuilderById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.deleteMultiplePcBuilderById();
        }
      });
    } else if (type === 'edit') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultiplePcBuilderById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.pcBuilderService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
