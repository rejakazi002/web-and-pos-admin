...
import {DATA_STATUS, TABLE_TAB_DATA, TABLE_TAB_OTHERS} from '../../../core/utils/app-data';
...
export class AllVendorComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private setPageData(): void {
    this.title.setTitle('Admin Control');
    this.pageDataService.setPageData({
      title: 'Admin Control',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'Admin Control', url: null},
      ]
    })
  }
  ...
  private getAllVendors() {
    ...
    const subscription = this.vendorDataService.getAllVendors(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleVendorById() {
    const subscription = this.vendorDataService.deleteMultipleVendorById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleVendorById(data: any) {
    const subscription = this.vendorDataService.updateMultipleVendorById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleVendorById(data);
        }
      });
    }
  }
