...
export class AllZoneComponent extends adminBaseMixin(Component) implements AfterViewInit, OnDestroy {

  // Store Data
  zones: Zone[] = [];
  private holdPrevUsers: Zone[] = [];
  ...
  private readonly zoneDataService = inject(ZoneService);
  ...
  ngAfterViewInit(): void {
    ...
    this.subSearch = formValue.pipe(
      ...
      switchMap(data => {
        ...
        return this.zoneDataService.getAllZones(filterData, this.searchQuery);
      })
    ).subscribe({
      next: res => {
        this.searchUsers = res.data;
        this.zones = this.searchUsers;
  ...
  private getAllZones() {
    ...
    this.subDataGetAll = this.zoneDataService.getAllZones(filterData, this.searchQuery)
      .subscribe({
        next: res => {
          this.zones = res.data;
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.zoneDataService.deleteMultipleZoneById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleUserById(data: any) {
    this.subDataUpdateMulti = this.zoneDataService.updateMultipleZoneById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
