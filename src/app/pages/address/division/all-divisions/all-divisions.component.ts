...
export class AllDivisionsComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private readonly divisionDataService = inject(DivisionService);
  ...
  private getAllDivisions() {
    ...
    const subscription = this.divisionDataService.getAllDivisions(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleDivisionById() {
    const subscription = this.divisionDataService.deleteMultipleDivisionById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleDivisionById(data: any) {
    const subscription = this.divisionDataService.updateMultipleDivisionById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
