...
export class SubscriptionReportComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private setPageData(): void {
    this.title.setTitle('Subscription Report');
    this.pageDataService.setPageData({
      title: 'Subscription Report',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'Subscription Report', url: null},
      ]
    })
  }
  ...
  /**
   * HTTP REQ HANDLE
   * getAllSubscriptionReport()
   * deleteMultipleCategoryById()
   * updateMultipleUserById()
   */

  private getAllSubscriptionReport() {
    ...
    const subscription = this.subscriptionDataService.getAllSubscriptionReport(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleCategoryById() {
    const subscription = this.subscriptionDataService.deleteMultipleSubscriptionById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCategoryById(data: any) {
    const subscription = this.subscriptionDataService.updateMultipleSubscriptionById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
