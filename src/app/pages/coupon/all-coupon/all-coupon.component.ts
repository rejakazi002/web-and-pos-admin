...
@Component({
  ...
  styleUrl: './all-coupon.component.scss'
})
export class AllCouponComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private setPageData(): void {
    ...
    this.pageDataService.setPageData({
      ...
      navArray: [
        ...
        {name: 'Coupon', url: 'https://www.youtube.com/embed/XgpQ4XMWkoo'},
      ]
    })
  }
  ...
  private getAllCategories() {
    ...
    const subscription = this.couponService.getAllCoupons(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleCouponById() {
    const subscription = this.couponService.deleteMultipleCouponById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCouponById(data: any) {
    const subscription = this.couponService.updateMultipleCouponById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  filterData(value: any, type: 'coupon', index: number,) {
    switch (type) {
      case 'coupon': {
        if (value) {
          this.filter = {...this.filter, ...{'coupon._id': value}};
          this.activeFilter1 = index;
        } else {
          delete this.filter['coupon._id'];
