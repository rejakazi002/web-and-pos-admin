...
@Component({
  selector: 'app-all-ready-landing-page',
  templateUrl: './all-ready-landing-page.component.html',
  styleUrl: './all-ready-landing-page.component.scss'
})
export class AllReadyLandingPageComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private readonly fixedLandingPageService = inject(ReadyLandingPageService);
  ...
  private setPageData(): void {
    ...
    this.pageDataService.setPageData({
      ...
      navArray: [
        ...
        {name: 'OfferPage', url: 'https://www.youtube.com/embed/HqWxM-PrTzg?si=6NayN2bUBnjT1YTe'},
      ]
    })
  }
  ...
  addLandingPage(): void {
    this.loading = true;
    this.fixedLandingPageService.addLandingPageFromGadget().subscribe({
      next: (res) => {
        this.loading = false;
        if (res.success) {
  ...
  private getAllOfferPages() {
    ...
    const subscription = this.fixedLandingPageService.getAllReadyLandingPages(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleOfferPageById() {
    const subscription = this.fixedLandingPageService.deleteMultipleReadyLandingPageById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleOfferPageById(data: any) {
    const subscription = this.fixedLandingPageService.updateMultipleReadyLandingPageById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteAllTrashByShop() {
    const subscription = this.fixedLandingPageService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
  ...
  private cloneSingleProduct(id: string) {
    const subscription = this.fixedLandingPageService.cloneSingleProduct(id)
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
  ...
  copyLink(data:any): void {
    const link = `https://${this.shopInformation?.domain}/promotion/${data}`; // Replace with your dynamic link
    this.clipboard.copy(link);
    this.uiService.message('Link copied successfully','success') ; // Optional feedback to the user
  }
  /**
   * Ui Essential
   * getStatusClass()
