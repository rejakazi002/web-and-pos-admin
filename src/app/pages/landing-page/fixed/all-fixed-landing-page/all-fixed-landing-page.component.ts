...
@Component({
  selector: 'app-all-fixed-landing-page',
  templateUrl: './all-fixed-landing-page.component.html',
  styleUrl: './all-fixed-landing-page.component.scss'
})
export class AllFixedLandingPageComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private readonly fixedLandingPageService = inject(FixedLandingPageService);
  ...
  private getAllOfferPages() {
    ...
    const subscription = this.fixedLandingPagService.getAllFixedLandingPages(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleOfferPageById() {
    const subscription = this.fixedLandingPagService.deleteMultipleFixedLandingPageById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleOfferPageById(data: any) {
    const subscription = this.fixedLandingPagService.updateMultipleFixedLandingPageById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteAllTrashByShop() {
    const subscription = this.fixedLandingPagService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
  ...
  private cloneSingleProduct(id: string) {
    const subscription = this.fixedLandingPagService.cloneSingleProduct(id)
      .subscribe({
        next: res => {
        if (res.success) {
          this.uiService.message(res.message, 'success');
  ...
  copyLink(data:any): void {
    const link = `https://${this.shopInformation?.domain}/offer/${data}`; // Replace with your dynamic link
