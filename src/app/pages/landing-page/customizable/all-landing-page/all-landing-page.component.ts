...
@Component({
  selector: 'app-all-landing-page',
  templateUrl: './all-landing-page.component.html',
  styleUrl: './all-landing-page.component.scss'
})
export class AllLandingPageComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private setPageData(): void {
    this.title.setTitle('LandingPage');
    this.pageDataService.setPageData({
      title: 'LandingPage',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'LandingPage', url:  'https://www.youtube.com/embed/otEQZ3oSQOY'},
      ]
    })
  }
  ...
  private getAllLandingPages() {
    ...
    const subscription = this.landingPageService.getAllLandingPages(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleLandingPageById() {
    const subscription = this.landingPageService.deleteMultipleLandingPageById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteMultiplePageById() {
    const subscription = this.landingPageService.deleteMultiplePageById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleLandingPageById(data: any) {
    const subscription = this.landingPageService.updateMultipleLandingPageById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
    ...
    if (type === 'delete') {
      if(this.selectedTab !== 'trash'){
        ...
        dialogRef.afterClosed().subscribe(dialogResult => {
          if (dialogResult) {
            this.deleteMultipleLandingPageById();
          }
        });
      }else{
        const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleLandingPageById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.landingPageService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
  ...
  copyLink(data:any): void {
    const link = `https://${this.shopInformation?.domain}/landing-page/${data}`; // Replace with your dynamic link
    this.clipboard.copy(link);
    this.uiService.message('Link copied successfully','success')
  }
  /**
   * Ui Essential
   * getStatusClass()
