...
@Component({
  selector: 'app-all-user-notification',
  templateUrl: './all-user-notification.component.html',
  styleUrl: './all-user-notification.component.scss'
})
export class AllUserNotificationComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: UserNotification[] = [];
  ...
  private setPageData(): void {
    this.title.setTitle('UserNotification');
    this.pageDataService.setPageData({
      title: 'UserNotification',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'UserNotification', url: null},
      ]
    })
  }
  ...
  private getAllUserNotification() {
    ...
    const subscription = this.userNotificationService.getAllUserNotification(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleUserNotificationById() {
    const subscription = this.userNotificationService.deleteMultipleUserNotificationById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleUserNotificationById(data: any) {
    const subscription = this.userNotificationService.updateMultipleUserNotificationById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.deleteMultipleUserNotificationById();
        }
      });
    } else if (type === 'edit') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleUserNotificationById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.userNotificationService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
