...
import {DATA_STATUS, DATA_STATUS1, TABLE_TAB_DATA, TABLE_TAB_DATA1} from "../../../core/utils/app-data";
...
@Component({
  ...
  styleUrl: './prescription-order.component.scss'
})
export class PrescriptionOrderComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: PrescriptionOrder[] = [];
  protected dataStatus: Select[] = DATA_STATUS1;
  protected tableTabData: Select[] = TABLE_TAB_DATA1;
  ...
  private readonly prescriptionOrderService = inject(PrescriptionOrderService);
  ...
  private setPageData(): void {
    this.title.setTitle('PrescriptionOrder');
    this.pageDataService.setPageData({
      title: 'PrescriptionOrder',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'PrescriptionOrder', url: null},
      ]
    })
  }
  ...
  private getAllPrescriptionOrder() {
    ...
    const subscription = this.prescriptionOrderService.getAllPrescriptionOrder(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultiplePrescriptionOrderById() {
    const subscription = this.prescriptionOrderService.deleteMultiplePrescriptionOrderById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultiplePrescriptionOrderById(data: any) {
    const subscription = this.prescriptionOrderService.updateMultiplePrescriptionOrderById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.deleteMultiplePrescriptionOrderById();
        }
      });
    } else if (type === 'edit') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultiplePrescriptionOrderById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.prescriptionOrderService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
