...
export class AllModelComponent extends DataTableSelectionBase(adminBaseMixin(Component)) implements AfterViewInit, OnDestroy {
  ...
  private getAllModels() {
    ...
    const subscription = this.modelService.getAllModels1(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleModelById() {
    const subscription = this.modelService.deleteMultipleModelById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
