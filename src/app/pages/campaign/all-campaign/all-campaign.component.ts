...
export class AllCampaignComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  protected categories: PromoOfferService[] = [];
  ...
  private readonly campaignService = inject(PromoOfferService);
  ...
  private setPageData(): void {
    this.title.setTitle('Campaign');
    this.pageDataService.setPageData({
      title: 'Campaign',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'Campaign', url: 'https://www.youtube.com/embed/XgpQ4XMWkoo'},
      ]
    })
  }
  ...
  private getAllCategories() {
    ...
    const subscription = this.campaignService.getAllPromoOffers(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleCampaignById() {
    const subscription = this.campaignService.deleteMultiplePromoOfferById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCampaignById(data: any) {
    const subscription = this.campaignService.updateMultiplePromoOfferById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  filterData(value: any, type: 'campaign', index: number,) {
    switch (type) {
      case 'campaign': {
        if (value) {
          this.filter = {...this.filter, ...{'campaign._id': value}};
          this.activeFilter1 = index;
        } else {
          delete this.filter['campaign._id'];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.deleteMultipleCampaignById();
        }
      });
    } else if (type === 'edit') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleCampaignById(data);
        }
      });
    } else if (type === 'trash') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
