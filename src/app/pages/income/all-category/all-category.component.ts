...
export class AllCategoryComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private deleteMultipleCategoryById() {
    const subscription = this.categoryService.deleteMultipleIncomeCategoryById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCategoryById(data: any) {
    const subscription = this.categoryService.updateMultipleIncomeCategoryById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
