...
@Component({
  ...
  styleUrl: './all-income.component.scss',
  providers: [DatePipe],
})
export class AllIncomeComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  allSummary: any;
  protected categories: IncomeCategory[] = [];
  ...
  private readonly incomeCategoryService = inject(IncomeCategoryService);
  ...
  private getAllCategories() {
    ...
    const subscription = this.incomeCategoryService.getAllCategories(filterData)
      .subscribe({
        next: res => {
  ...
  load() {
    ...
    this.incomeService.summary(params).subscribe(res => this.summary = res.data);
    this.incomeService.report(params).subscribe(res => this.report = res.data);
  ...
  private getAllAllIncomes() {
    ...
    const subscription = this.incomeService.getAllIncomes(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleIncomeById() {
    const subscription = this.incomeService.deleteMultipleIncomeById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleIncomeById(data: any) {
    const subscription = this.incomeService.updateMultipleIncomeById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  filterData(value: any, type: 'income', index: number,) {
    switch (type) {
      case 'income': {
        if (value) {
          this.filter = {...this.filter, ...{'income._id': value}};
          this.activeFilter1 = index;
        } else {
          delete this.filter['income._id'];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleIncomeById(data);
        }
      });
    }
  }
  ...
  printReport(): void {
    ...
    printWindow.document.write(`
      ...
      <html>
        <head>
          <title>Income Report - ${this.businessInfo.name}</title>
          <meta charset="utf-8">
          <style>
