...
export class AddCategoryComponent extends adminBaseMixin(Component) implements OnInit, OnDestroy {
  ...
  private getCategoryById() {
    const subscription = this.categoryService.getIncomeCategoryById(this.id).subscribe({
      next: (res) => {
        if (res.data) {
  ...
  private addCategory() {
    const subscription = this.categoryService
      .addIncomeCategory(this.dataForm.value)
      .subscribe({
        next: (res) => {
          if (res.success) {
  ...
  private updateCategoryById() {
    const subscription = this.categoryService
      .updateIncomeCategoryById(this.category._id, this.dataForm.value)
      .subscribe({
        next: (res) => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
