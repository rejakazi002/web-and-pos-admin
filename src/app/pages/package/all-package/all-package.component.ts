...
@Component({
  ...
  styleUrl: './all-package.component.scss'
})
export class AllPackageComponent extends adminBaseMixin(Component) implements AfterViewInit, OnDestroy {

  // Store Data
  packages: Package[] = [];
  private holdPrevUsers: Package[] = [];
    ...
    this.subSearch = formValue.pipe(
      ...
      switchMap(data => {
        ...
        if (this.searchQuery === '' || this.searchQuery === null) {
          this.searchUsers = [];
          this.packages = this.holdPrevUsers;
        ...
        return this.packageDataService.getAllPackage(filterData, this.searchQuery);
      })
    ).subscribe({
      next: res => {
        this.searchUsers = res.data;
        this.packages = this.searchUsers;
  ...
  private getAllPackage() {
    ...
    this.subDataGetAll = this.packageDataService.getAllPackage(filterData, this.searchQuery)
      .subscribe({
        next: res => {
          this.packages = res.data;
          if (this.packages && this.packages.length) {
            this.packages.forEach((m, i) => {
              const index = this.selectedIds.findIndex(f => f === m._id);
              this.packages[i].select = index !== -1;
            });
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.packageDataService.deleteMultiplePackageById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleUserById(data: any) {
    this.subDataUpdateMulti = this.packageDataService.updateMultiplePackageById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  onAllSelectChange(event: MatCheckboxChange) {
    const currentPageIds = this.packages.map(m => m._id);
    if (event.checked) {
      ...
      currentPageIds.forEach(m => {
        this.packages.find(f => f._id === m).select = false;
