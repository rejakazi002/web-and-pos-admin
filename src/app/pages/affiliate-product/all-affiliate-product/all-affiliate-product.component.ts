...
@Component({
  selector: 'app-all-affiliate-product',
  templateUrl: './all-affiliate-product.component.html',
  styleUrl: './all-affiliate-product.component.scss'
})
export class AllAffiliateProductComponent extends adminBaseMixin(Component) implements AfterViewInit, OnDestroy {

  // Store Data
  affiliateProducts: AffiliateProduct[] = [];
  private holdPrevUsers: AffiliateProduct[] = [];
  ...
  navArray: NavBreadcrumb[] = [
    ...
    {name: 'All AffiliateProduct', url: null},
  ];
  ...
  private readonly affiliateProductDataService = inject(AffiliateProductService);


  ngOnInit() {
    // Reload Data
    this.subReload = this.reloadService.refreshData$.subscribe(() => {
      this.getAllAffiliateProducts();
    })
    ...
    this.subSearch = formValue.pipe(
      ...
      switchMap(data => {
        ...
        if (this.searchQuery === '' || this.searchQuery === null) {
          this.searchUsers = [];
          this.affiliateProducts = this.holdPrevUsers;
        ...
        return this.affiliateProductDataService.getAllAffiliateProducts(filterData, this.searchQuery);
      })
    ).subscribe({
      next: res => {
        this.searchUsers = res.data;
        this.affiliateProducts = this.searchUsers;
   ...
   */
  private getAllAffiliateProducts() {
    const pagination: Pagination = {
      pageSize: Number(this.dataPerPage),
      currentPage: Number(this.currentPage) - 1
    };
    ...
    this.subDataGetAll = this.affiliateProductDataService.getAllAffiliateProducts(filterData, this.searchQuery)
      .subscribe({
        next: res => {
          this.isLoading = false;
          this.affiliateProducts = res.data;
          if (this.affiliateProducts && this.affiliateProducts.length) {
            this.affiliateProducts.forEach((m, i) => {
              const index = this.selectedIds.findIndex(f => f === m._id);
              this.affiliateProducts[i].select = index !== -1;
            });
  ...
  private updateMultipleUserById(data: any) {
    this.subDataUpdateMulti = this.affiliateProductDataService.updateMultipleAffiliateProductById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteMultipleUserById() {
    this.subDataDeleteMulti = this.affiliateProductDataService.deleteMultipleAffiliateProductById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private checkSelectionData() {
    ...
    this.affiliateProducts.forEach(m => {
      if (!m.select) {
        isAllSelect = false;
      }
    });
  ...
  onAllSelectChange(event: MatCheckboxChange) {
    const currentPageIds = this.affiliateProducts.map(m => m._id);
    if (event.checked) {
      ...
      currentPageIds.forEach(m => {
        this.affiliateProducts.find(f => f._id === m).select = false;
