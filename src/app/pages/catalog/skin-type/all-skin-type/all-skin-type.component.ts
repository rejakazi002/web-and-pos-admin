...
@Component({
  selector: 'app-all-skin-type',
  templateUrl: './all-skin-type.component.html',
  styleUrl: './all-skin-type.component.scss'
})
export class AllSkinTypeComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private setPageData(): void {
    this.title.setTitle('SkinType');
    this.pageDataService.setPageData({
      title: 'SkinType',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'SkinType', url: null},
      ]
    })
  }
  ...
  private getAllSkinTypes() {
    // console.log('getAllSkinTypes')
    ...
    const subscription = this.skinTypeService.getAllSkinTypes(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleSkinTypeById() {
    const subscription = this.skinTypeService.deleteMultipleSkinTypeById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleSkinTypeById(data: any) {
    const subscription = this.skinTypeService.updateMultipleSkinTypeById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.deleteMultipleSkinTypeById();
        }
      });
    } else if (type === 'edit') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleSkinTypeById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.skinTypeService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
