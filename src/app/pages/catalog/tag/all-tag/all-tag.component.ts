...
@Component({
  ...
  styleUrl: './all-tag.component.scss'
})
export class AllTagComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private setPageData(): void {
    ...
    this.pageDataService.setPageData({
      ...
      navArray: [
        ...
        {name: 'Tag', url: 'https://www.youtube.com/embed/LhEJ52APuRs?si=ypCrfKVsE-haDAos'},
      ]
    })
  }
  ...
  private getAllTags() {
    ...
    const subscription = this.tagService.getAllTags(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleTagById() {
    const subscription = this.tagService.deleteMultipleTagById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleTagById(data: any) {
    const subscription = this.tagService.updateMultipleTagById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteAllTrashByShop() {
    const subscription = this.tagService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
