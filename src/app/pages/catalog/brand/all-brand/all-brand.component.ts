...
@Component({
  ...
  styleUrl: './all-brand.component.scss'
})
export class AllBrandComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private updateMultipleBrandById(data: any) {
    const subscription = this.brandService.updateMultipleBrandById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteAllTrashByShop() {
    const subscription = this.brandService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
