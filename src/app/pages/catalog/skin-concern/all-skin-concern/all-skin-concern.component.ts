...
@Component({
  selector: 'app-all-skin-concern',
  templateUrl: './all-skin-concern.component.html',
  styleUrl: './all-skin-concern.component.scss'
})
export class AllSkinConcernComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: SubCategory[] = [];
  ...
  private setPageData(): void {
    this.title.setTitle('SkinConcern');
    this.pageDataService.setPageData({
      title: 'SkinConcern',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'SkinConcern', url: null},
      ]
    })
  }
  ...
  private getAllSkinConcerns() {
    // console.log('getAllSkinConcerns')
    ...
    const subscription = this.skinConcernService.getAllSkinConcerns(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleSkinConcernById() {
    const subscription = this.skinConcernService.deleteMultipleSkinConcernById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleSkinConcernById(data: any) {
    const subscription = this.skinConcernService.updateMultipleSkinConcernById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.deleteMultipleSkinConcernById();
        }
      });
    } else if (type === 'edit') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleSkinConcernById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.skinConcernService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
