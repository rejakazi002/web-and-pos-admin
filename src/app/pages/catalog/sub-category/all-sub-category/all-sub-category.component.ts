...
@Component({
  ...
  styleUrl: './all-sub-category.component.scss'
})
export class AllSubCategoryComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private setPageData(): void {
    this.title.setTitle('Sub Category');
    this.pageDataService.setPageData({
      title: 'Sub Category',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'Sub Category', url: null},
      ]
    })
  }
  ...
  private getAllSubCategories() {
    ...
    const subscription = this.subCategoryService.getAllSubCategories(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleSubCategoryById() {
    const subscription = this.subCategoryService.deleteMultipleSubCategoryById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleSubCategoryById(data: any) {
    const subscription = this.subCategoryService.updateMultipleSubCategoryById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteAllTrashByShop() {
    const subscription = this.subCategoryService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
