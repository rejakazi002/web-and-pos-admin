...
export class LandingPage2Component implements OnInit , OnDestroy {
  ...
  ngOnInit() {
    ...
    if (this.isPreviewMode && this.previewData) {
      this.setupPreviewMode();
      // Skip API calls in preview mode to avoid CORS errors
      return;
    } else {
      // Normal mode - get data from slug
    ...
    // Only make API calls if not in preview mode
    if (isPlatformBrowser(this.platformId) && !this.isPreviewMode) {
      this.getChatLink();
      this.getShopInfo();
    }
  ...
  private setupPreviewMode() {
    // Set up the component with preview data
    this.singleLandingPage = this.previewData;
    this.product = this.previewData.product || null;
    
    // Set default values for preview mode to avoid undefined errors
    this.shopInfo = {
      name: 'Preview Shop',
      logo: null,
      socialLinks: []
    ...
    // Load all components immediately for preview
    this.showLazyComponent = ['sec1', 'sec2', 'sec3', 'sec4', 'sec5', 'sec6' , 'sec7' , 'sec8' , 'sec9','sec10'];
  }
  ...
  private getShopInfo() {
    const subscription = this.shopInfoService.getShopInformation() .subscribe({
      ...
      error: err => {
        // Silently handle error in preview mode or development
        this.shopInfo = {
          name: 'Shop',
          logo: null,
          socialLinks: []
