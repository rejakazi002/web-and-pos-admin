...
@Component({
  selector: 'app-all-blog-comment',
  templateUrl: './all-blog-comment.component.html',
  styleUrl: './all-blog-comment.component.scss'
})
export class AllBlogCommentComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: BlogComment[] = [];
  ...
  private setPageData(): void {
    this.title.setTitle('BlogComment');
    this.pageDataService.setPageData({
      title: 'BlogComment',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'BlogComment', url: null},
      ]
    })
  }
  ...
  private getAllBlogComment() {
    ...
    const subscription = this.blogCommentService.getAllBlogComment(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleBlogCommentById() {
    const subscription = this.blogCommentService.deleteMultipleBlogCommentById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleBlogCommentById(data: any) {
    const subscription = this.blogCommentService.updateMultipleBlogCommentById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.deleteMultipleBlogCommentById();
        }
      });
    } else if (type === 'edit') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleBlogCommentById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.blogCommentService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
