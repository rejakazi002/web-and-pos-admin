...
import {Booking} from "../../../interfaces/common/booking.interface";
import {Select} from "../../../interfaces/core/select";
import {DATA_STATUS, DATA_STATUS2, TABLE_TAB_DATA, TABLE_TAB_DATA2} from "../../../core/utils/app-data";
...
export class BookingListComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: Booking[] = [];
  protected dataStatus: Select[] = DATA_STATUS2;
  protected tableTabData: Select[] = TABLE_TAB_DATA2;
   ...
   * getAllCategories()
   * getAllBooking()
   * deleteMultipleBookingById()
   * updateMultipleBookingById()
   */
  ...
  private getAllBooking() {
    ...
    const subscription = this.bookingService.getAllBooking(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleBookingById() {
    const subscription = this.bookingService.deleteMultipleBookingById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleBookingById(data: any) {
    const subscription = this.bookingService.updateMultipleBookingById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.deleteMultipleBookingById();
        }
      });
    } else if (type === 'edit') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleBookingById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.bookingService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
