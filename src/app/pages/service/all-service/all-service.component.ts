...
export class AllServiceComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: Service[] = [];
  ...
  private getAllService() {
    ...
    const subscription = this.serviceService.getAllService(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleServiceById() {
    const subscription = this.serviceService.deleteMultipleServiceById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleServiceById(data: any) {
    const subscription = this.serviceService.updateMultipleServiceById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.deleteMultipleServiceById();
        }
      });
    } else if (type === 'edit') {
      const dialogRef = this.dialog.open(ConfirmDialogComponent, {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleServiceById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.serviceService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
