...
@Component({
  ...
  styleUrl: './all-seo-page.component.scss'
})
export class AllSeoPageComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: SeoPage[] = [];
  ...
  private getAllSeoPage() {
    ...
    const subscription = this.seoPageService.getAllSeoPage(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleSeoPageById() {
    const subscription = this.seoPageService.deleteMultipleSeoPageById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleSeoPageById(data: any) {
    const subscription = this.seoPageService.updateMultipleSeoPageById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleSeoPageById(data);
        }
      });
    }
    else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.seoPageService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
