...
import {DATA_STATUS, MAX_UPLOAD, TABLE_TAB_DATA} from "../../../../core/utils/app-data";
...
@Component({
  ...
  styleUrl: './all-carousel.component.scss'
})
export class AllCarouselComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: Carousel[] = [];
  ...
  sortOrder: number = -1; // Default sorting order (-1)
  ...
  private setPageData(): void {
    this.title.setTitle('Carousel');
    this.pageDataService.setPageData({
      title: 'Carousel',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'Carousel', url: 'https://www.youtube.com/embed/szVMgCZvnTw'},
      ]
    })
  }
  ...
  handleRouter(){
    this.router.navigate(['/customization/add-carousel'])
    // console.log("total carousel data,", this.totalData);
  ...
  private deleteMultipleCarouselById() {
    const subscription = this.carouselService.deleteMultipleCarouselById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCarouselById(data: any) {
    const subscription = this.carouselService.updateMultipleCarouselById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteAllTrashByShop() {
    const subscription = this.carouselService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
