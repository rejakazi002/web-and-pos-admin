...
@Component({
  ...
  styleUrl: './all-banner.component.scss'
})
export class AllBannerComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: Banner[] = [];
  ...
  private getAllBanner() {
    ...
    const subscription = this.bannerService.getAllBanner(filterData, this.searchQuery)
      .subscribe({
        next: res => {
  ...
  private deleteMultipleBannerById() {
    const subscription = this.bannerService.deleteMultipleBannerById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleBannerById(data: any) {
    const subscription = this.bannerService.updateMultipleBannerById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  public openConfirmDialog(type: string, data?: any) {
    if (type === 'delete') {
      ...
      dialogRef.afterClosed().subscribe(dialogResult => {
        if (dialogResult) {
          this.updateMultipleBannerById(data);
        }
      });
    }
  else if (type === 'trash') {
  ...
  private deleteAllTrashByShop() {
    const subscription = this.bannerService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
