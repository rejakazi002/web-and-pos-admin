...
import {DATA_STATUS, MAX_POPUP_UPLOAD, MAX_UPLOAD, TABLE_TAB_DATA} from "../../../../core/utils/app-data";
...
@Component({
  ...
  styleUrl: './all-popup.component.scss'
})
export class AllPopupComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {

  // Decorator
  @ViewChild('searchForm', {static: true}) private searchForm: NgForm;

  // Store Data
  override allTableData: Popup[] = [];
  ...
  private setPageData(): void {
    this.title.setTitle('Popup');
    this.pageDataService.setPageData({
      title: 'Popup',
      navArray: [
        {name: 'Dashboard', url: `/dashboard`},
        {name: 'Popup', url: 'https://www.youtube.com/embed/XHh-ptgkxDU'},
      ]
    })
  }
  ...
  handleRouter(){
    this.router.navigate(['/customization/add-popup'])
  ...
  private deleteMultiplePopupById() {
    const subscription = this.popupService.deleteMultiplePopupById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultiplePopupById(data: any) {
    const subscription = this.popupService.updateMultiplePopupById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private deleteAllTrashByShop() {
    const subscription = this.popupService.deleteAllTrashByShop()
      .subscribe({
        next: res => {
          if (res.success) {
            this.uiService.message(res.message, 'success');
