...
export class AllCategoryComponent extends DataTableSelectionBase(Component) implements AfterViewInit, OnDestroy {
  ...
  private deleteMultipleCategoryById() {
    const subscription = this.categoryService.deleteMultipleExpenseCategoryById(this.selectedIds)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
  ...
  private updateMultipleCategoryById(data: any) {
    const subscription = this.categoryService.updateMultipleExpenseCategoryById(this.selectedIds, data)
      .subscribe({
        next: res => {
          if (res.success) {
            this.selectedIds = [];
