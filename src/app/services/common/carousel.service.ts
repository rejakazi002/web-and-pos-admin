...
export class CarouselService {
  ...
  getAllCarousel(filterData: FilterData, searchQuery?: string) {
    let params = new HttpParams();
    if (searchQuery) {
      params = params.append('q', searchQuery);
    }
    return this.httpClient.post<{ data: Carousel[], count: number, success: boolean }>(API_CAROUSEL + 'get-all-by-shop', filterData, {params});
  }
  ...
  getCarouselById(id: string, select?: string) {
    ...
    return this.httpClient.get<{ data: Carousel, message: string, success: boolean }>(API_CAROUSEL + 'get-by-id/' + id, {params});
  }
  ...
  deleteAllTrashByShop( checkUsage?: boolean) {
    ...
    return this.httpClient.post<ResponsePayload>(API_CAROUSEL + 'delete-all-trash-by-shop', {params});
  }
}
