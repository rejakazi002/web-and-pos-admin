...
export class TagService {
  ...
  getAllTags(filterData: FilterData, searchQuery?: string) {
    ...
    return this.httpClient.post<{ data: Tag[], count: number, success: boolean }>(API_TAG + 'get-all-by-shop', filterData, {params});
  }
  ...
  getTagById(id: string, select?: string) {
    ...
    return this.httpClient.get<{ data: Tag, message: string, success: boolean }>(API_TAG + 'get-by-id/' + id, {params});
  }
  ...
  deleteAllTrashByShop( checkUsage?: boolean) {
    ...
    return this.httpClient.post<ResponsePayload>(API_TAG + 'delete-all-trash-by-shop', {params});
  }
}
