...
export function adminBaseMixin<T extends Constructor>(Base: T) {
  return class extends Base {
    ...
    get checkAddPermission(): boolean {
      return this.permissions?.includes(AdminPermissions.CREATE) ?? false;
    }

    get checkDeletePermission(): boolean {
      return this.permissions?.includes(AdminPermissions.DELETE) ?? false;
    }

    get checkEditPermission(): boolean {
      return this.permissions?.includes(AdminPermissions.EDIT) ?? false;
    }
